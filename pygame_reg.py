...
class InfoBox:
	def __init__(self,x,y,w,h, title, value):
		self.label = TextObject(x,y,w/2,h, text = title)
		self.value = value
		self.valueBox = TextObject(x+w/2,y,w/2,h, text = str(value))

	def setvalue(self,value):
		self.value = value
		self.valueBox.text = str(value)

	def draw(self,screen):
		self.label.draw(screen)
		self.valueBox.draw(screen)
